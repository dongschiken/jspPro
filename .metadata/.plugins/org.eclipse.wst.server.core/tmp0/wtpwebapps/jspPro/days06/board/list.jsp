     ...
     <script>
       ...
       $("#cmbNumberPerPage").on("change", function (){
          var selectedValue = $(this).val(); // 선택된 옵션 값
           //alert(selectedValue);
           location.href = `/jspPro/board/list.do?currentpage${pDto.currentPage}=&numberPerPage=\${selectedValue}`
				...
				<tr>
					...
					<div class=pagination>
						...
						<c:choose>
							...
							<c:otherwise>
							<a href="<%= contextPath %>/board/list.do?currentpage=${ page_num }">${ page_num }</a>	
							</c:otherwise>
						</c:choose>						
						</c:forEach>
						<c:if test="${ pDto.next }">
